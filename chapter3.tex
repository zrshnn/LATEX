...
\begin{itemize}
	...
	\item Solar Energy Collection and Storage - PV panels capture solar energy, which is stored in a \ce{LiFePO4}  battery through a solar charge controller and DC breakers for protection.
	
	\item Energy Conversion - The stored DC power is converted to AC using a power inverter to supply household appliances.
	
	\item User Interface and Control - The ESP32 microcontroller connects with a mobile app via a router and Firebase, allowing users to monitor battery levels and manage rental requests.
	
	\item Power Access and Management - A coin slot and relay control access to the power source, while an LCD displays real-time system status.
	
	\item GPS Tracking and Security -  A GPS and GSM module track the power source’s location, ensuring security and preventing theft.
	
	\item Admin Control - Admins can monitor and manage the system through a mobile interface, overseeing usage and system health.
