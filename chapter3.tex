...
\subsection{System Requirements}
	...
	& - The system shall provide  status updates on the mobile app, including remaining battery percentage, and device location. \\
...
The schedule of load includes three main loads, namely Electric Fan, Mobile Phone Charger, and a 5W auxiliary load. Each load are multiplier to hours of operations and their products are summed up to get the total watt-hour daily consumption.

\subsubsection{Battery Sizing}

\begin{itemize}
	\item\textbf{ Detachable Power Source:} In order to calculate the battery capacity required, the daily consumption must be divided by the system voltage (for this study, 12V):
	...
	\item\textbf{ Station:} In order to calculate the battery capacity required in station, the battery capacity in the detachable power source must be multiplied in how many detachable power source (for this study prototype, 1), then we need to add also the 35\% of system loss according to chuchu and add also the 0.858Ah for the consumption of the ESP32.
	
	\begin{align} 
		\text{Battery Capacity} &= (\text{Battery Capacity Detachable Power Source} + 35\%) + 0.858Ah \nonumber\\
		& = 68.358Ah \nonumber \\
		\text{Battery Capacity} &\approx 70 \text{Ah}
	\end{align}
	
	Use: 70Ah LifePO4 Battery.
	
	\item\textbf{ Panel Sizing:} Calculation are done in Accordance to PEC (Philippine Electric Code 2017).
	
	\begin{align}
		\text{PV Power} &= \frac{Total Daily Consumption}{Sun Peak Hours} \nonumber\\
		& = \frac{500Wh}{3.5h} \nonumber \\
		& = 142.857W\nonumber\\
		\text{PV Power} &= 200W
	\end{align}
	...
	\begin{align}
		I_{cc} &= \frac{Total PV Power}{System Voltage} \nonumber\\
		&= \frac{200W}{12V} \nonumber \\
		&=16.67A \nonumber \\
		I_{cc} &= 20A
	\end{align}
